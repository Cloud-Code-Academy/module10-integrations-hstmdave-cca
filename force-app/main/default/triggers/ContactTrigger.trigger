...
trigger ContactTrigger on Contact(before insert, after insert, before update, after update, before delete, after delete, after undelete) {
	...
	switch on Trigger.operationType{
        when BEFORE_INSERT {
			ContactTriggerHandler.jsonIdIsNull(Trigger.new);
        }
        when BEFORE_UPDATE {
        ...
        when AFTER_INSERT {
        	ContactTriggerHandler.jsonIdLessThan101(Trigger.new);
        }
        when AFTER_UPDATE {
        	ContactTriggerHandler.jsonIdGreaterThan100(Trigger.new, Trigger.oldMap);
        }
        when AFTER_DELETE {
